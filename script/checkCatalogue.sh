...
if [ $validate == "yes" ]; then
  ...
  fi
  {
    ...
    while IFS=, read -r code file; do
      ...
      if echo "$code" | grep -P '^2'; then
        frictionless validate --buffer-size 250000 --sample-size 750 --json "$folder"/../output/"$name"/rawdata/download/"$file".csv | jq -c '.|= .+ {file:"'"$file"'"}' >>"$folder"/../output/"$name"/processing/validate.jsonl
      else
        echo "non validare"
      fi
    done
